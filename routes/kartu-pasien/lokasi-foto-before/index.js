...
router.get("/", async function (req, res, next) {
  try {
    ...
    CONVERT(date, TglActivitas) as dateTglActivitas,
    CONVERT(time, TglActivitas) as timeTglActivitas,
    CONVERT(date, JamActivitas) as dateJamActivitas,
    CONVERT(time, JamActivitas) as timeJamActivitas,
    LokasiFotoBefore,
  ...
  `;
    ...
    if (querydata[0]) {
      ...
      querydata.forEach((items) => {
        dataTcard += `(
          '${items.NoAuto}','${items.NoAutoPerawatan}','${items.Keterangan}',
          '${items.UserEntry}','${items.LoginComp}','${items.CompName}',
          '${items.dateTglActivitas} ${items.timeTglActivitas}',
          '${items.dateJamActivitas} ${items.timeJamActivitas}',
